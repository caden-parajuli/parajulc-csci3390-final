...
object clustering_verifier {
  def main(args: Array[String]) {
    ...
    val original_vertices =
      Graph.fromEdges[Int, Int](graph_edges, 0).vertices.keys
    if (
      !(original_vertices
        .subtract(graph_vertices.keys)
        .isEmpty() && graph_vertices.keys.subtract(original_vertices).isEmpty())
    ) {
      println(
        "The set of vertices in the solution file does not match that of the input file."
      )
      sys.exit(1)
    }
    ...
    val graph_vertices3 = graph_vertices2
      .join(graph_vertices.map(x => (x._2, 1L)).reduceByKey(_ + _))
      .map({ case (a, x) => (x._1, (a, x._2)) })

    val graph2: Graph[Tuple2[Long, Long], Int] =
      Graph(graph_vertices3, graph_edges, (0L, 0L))
